...
class RobcoreNetAtmo {
	...
	*********************************************************/
	...
	PLUVIOMETRO
	*********************************************************/
	    function rnpfw_rain1h($id_modulo, $id_stazione, $oggetto_meteo) {	    	
        	global $wpdb;
        	$tabella_prefixata = $wpdb->prefix.'robcore_netatmo_data';
        	$timestamp_ultima_lettura = $wpdb->get_row("SELECT time_misura FROM $tabella_prefixata WHERE id_modulo = '$id_modulo' AND valore_misurato = 'rain1h' ORDER BY time_misura DESC LIMIT 1", ARRAY_N);
        	$timestamp_ultima_lettura = $timestamp_ultima_lettura[0];
        	$timestamp_ora = date("Y-m-d H:i:s");
        	// echo "ora: " . $timestamp_ora . "<br>";
        	// echo "misurata ultima: " . $timestamp_ultima_lettura . "<br>";
        	...
        	if ((strtotime($timestamp_ora)) - (strtotime($timestamp_ultima_lettura)) > 300) {
			            ...
			            $params_modulo = array("scale" =>"1hour",
			                            "type"=>"sum_rain",
			                            "date_end"=>"last",
			                            "device_id"=>$id_stazione,
			                            "module_id"=>$id_modulo,
			            ...
			            $dati_modulo = $oggetto_meteo->api("getmeasure", $params_modulo);
			            if (isset($dati_modulo)) {
			            	$rain1h = round($dati_modulo[0]["value"][0][0],1);
											$result = $wpdb->insert( 
												$wpdb->prefix . "robcore_netatmo_data", 
												array( 
													'misurazione' => $rain1h,
				...
				global $wpdb;
				$tabella_prefixata = $wpdb->prefix.'robcore_netatmo_data';
				$rain1h = $wpdb->get_row("SELECT misurazione FROM $tabella_prefixata WHERE id_modulo = '$id_modulo' AND valore_misurato = 'rain1h' ORDER BY time_misura DESC LIMIT 1", ARRAY_N);
				$rain1h = $rain1h[0];
				return $rain1h;
			}
		}
	    function rnpfw_rainToday($id_modulo, $id_stazione, $oggetto_meteo) {	    	
        	global $wpdb;
        	$tabella_prefixata = $wpdb->prefix.'robcore_netatmo_data';
        	$timestamp_ultima_lettura = $wpdb->get_row("SELECT time_misura FROM $tabella_prefixata WHERE id_modulo = '$id_modulo' AND valore_misurato = 'rainToday' ORDER BY time_misura DESC LIMIT 1", ARRAY_N);
        	$timestamp_ultima_lettura = $timestamp_ultima_lettura[0];
        	$timestamp_ora = date("Y-m-d H:i:s");
        	// echo "ora: " . $timestamp_ora . "<br>";
        	// echo "misurata ultima: " . $timestamp_ultima_lettura . "<br>";
        	...
        	if ((strtotime($timestamp_ora)) - (strtotime($timestamp_ultima_lettura)) > 300) {
			            ...
			            $params_modulo = array("scale" =>"1day",
			                            "type"=>"sum_rain",
			                            "date_end"=>"last",
			                            "device_id"=>$id_stazione,
			                            "module_id"=>$id_modulo,
			            ...
			            $dati_modulo = $oggetto_meteo->api("getmeasure", $params_modulo);
			            if (isset($dati_modulo)) {
			            	$rainToday = round($dati_modulo[0]["value"][0][0],1);
											$result = $wpdb->insert( 
												$wpdb->prefix . "robcore_netatmo_data", 
												array( 
													'misurazione' => $rainToday,
													'time_misura' => $timestamp_ora,
													'id_device' => $id_stazione,
													'id_modulo' => $id_modulo,
													'valore_misurato' => "rainToday"
												)
											 );
			            	return $rainToday;
			            }
				...
				global $wpdb;
				$tabella_prefixata = $wpdb->prefix.'robcore_netatmo_data';
				$rainToday = $wpdb->get_row("SELECT misurazione FROM $tabella_prefixata WHERE id_modulo = '$id_modulo' AND valore_misurato = 'rainToday' ORDER BY time_misura DESC LIMIT 1", ARRAY_N);
	...
	*********************************************************/
	...
	function rnpfw_ottieniIDElementi($tipo_elemento, $oggetto_meteo) {
		...
		if ($tipo_elemento == "modulo") {
			foreach ($listaDevice['modules'] as $modulo) { // parso tutti i moduli
				if (($modulo["type"] == "NAModule1") || ($modulo["type"] == "NAModule4")) { // prendo solo i moduli temp/hum interni ed esterni
		...
		if ($tipo_elemento == "pluvio") {
			foreach ($listaDevice['modules'] as $modulo) { // parso tutti i moduli
				if ($modulo["type"] == "NAModule3") { // prendo solo i moduli temp/hum interni ed esterni
					$id_modulo = $modulo["_id"];
					$nome_modulo = $modulo["module_name"];
					$id_stazione = $modulo["main_device"];
					$array_id_modulo_stazione = array($id_modulo,$nome_modulo,$id_stazione);
